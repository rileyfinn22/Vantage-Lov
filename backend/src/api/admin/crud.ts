...
const COMPANY_SCOPED_TABLES = new Set(["company", "salespeople", "teams", "interactions", "flags", "company_user_roles"]);
...
const app = new Hono<AuthVariable<false> & { Variables: { adminCompanyId: number | null } }>()
	.use("*", async (c, next) => {
		...
		logger.info({ userId: currentUser.id, email: currentUser.email, adminCompanyId }, "Admin CRUD request - company context");
	...
	.get("/:resource/:id", createParamValidator(resourceWithIdSchema, "Invalid resource or ID parameter"), async (c) => {
		...
		if (adminCompanyId !== null && COMPANY_SCOPED_TABLES.has(resource)) {
			...
			if (resource === "company" && record.id !== adminCompanyId) {
